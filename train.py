...
def valid(args, model, writer, test_loader, global_step):
    ...
    epoch_iterator = tqdm(test_loader,
                          ...
                          dynamic_ncols=True,
                          disable=(jittor.rank != 0))
    loss_fct = jittor.nn.CrossEntropyLoss()
    for step, batch in enumerate(epoch_iterator):
        batch = tuple(jittor.to(t, args.device) for t in batch)
        x, y = batch
        with jittor.no_grad():
            logits = model(x)
    ...
    accuracy = jittor.to(jittor.array(accuracy), args.device)
    # dist.barrier()
    if jittor.in_mpi:
        val_accuracy = accuracy.mpi_all_reduce("mean")
    else:
...
def train(args, model):
    ...
    while True:
        ...
        # dist.barrier()
        if jittor.in_mpi:
            train_accuracy = accuracy.mpi_all_reduce("mean")
        else:
