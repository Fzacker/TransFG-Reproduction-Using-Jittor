...
from jittor.nn import  CrossEntropyLoss, Dropout, softmax, Linear, Conv, LayerNorm
import collections
...
class Part_Attention(nn.Module):
    ...
    def execute(self, x):
        ...
        max_inx, _ = jittor.arg_reduce(last_map, "max", 2, keepdims=False)
        return _, max_inx
...
class Encoder(nn.Module):
    ...
    def execute(self, hidden_states):
        ...
        parts = parts.squeeze(1) if list(parts.shape)[1] == 1 else parts
...
def con_loss(features, labels):
    B, _ = features.shape
    alpha = 0
    features = jittor.misc.normalize(features)
    cos_matrix = jittor.nn.matmul(features, features.transpose())
